...
void main() {
    ...
    float qs = floor(u_resolution.y/2.);
    vec2 uvq = uv;
    float y1 = floor(uvq.y*qs)/qs;
    float y2 = floor(uvq.y*qs + 1.)/qs;
    float sce = (uvq.y-y1)/(y2-y1);
    sce = smoothstep(0., 1., sce);
    sce = smoothstep(0., 1., sce);
    sce = smoothstep(0., 1., sce);
    uvq.y = y1 + sce*(y2 - y1);
    ...
    vec2 uvrd = uvq + .622*ff*vec2(1.5, 1.) + 0.*333.5*ff*vec2(1., 0.)/u_resolution*2.;
    vec2 uvgd = uvq - .22*ff*vec2(1.5, 1.) + 0.*ff*vec2(1., 0.)/u_resolution*2.;
    vec2 uvbd = uvq + .11*ff*vec2(1.5, 1.) - 0.*333.5*ff*vec2(1., 0.)/u_resolution*2.;

    //vec4 mask = texture2D(tex2, uvgd) * u_usemask + vec4(1.) * (1.-u_usemask);
    //vec4 maskrr = texture2D(tex2, uvrd);
    //vec4 maskgg = texture2D(tex2, uvgd);
    //vec4 maskbb = texture2D(tex2, uvbd);
    //vec4 mask = vec4(maskrr.r, maskgg.g, maskbb.b, 1.);
    vec4 mask = texture2D(tex2, uvgd - vec2(1., 0.)/u_resolution);
    vec4 maskr = texture2D(tex2, uvgd + vec2(1., 0.)/u_resolution);
    //vec4 maskb = mask;
    //maskb *= 0.;
    ...
    vec4 blurred_original = texture2D(tex1, uvq);
    ...
    outc.rgb += mask.rgb*.36;
    outc.rgb -= maskr.rgb*.36;
    ...
    float salt = randomNoise(mod(uv+seed/1000000.+.3143+u_time*.0000+fbm(uv)*.002, .1));
